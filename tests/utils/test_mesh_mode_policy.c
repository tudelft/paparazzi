...
int main(void)
{
  ...
  input.clock_synchronized = false;
  expect(!mesh_mode_should_use_solo(&input), "unsafe clock selected solo");
  puts("ok 2 - clock loss selects mesh");
  input.clock_synchronized = true;
  input.self_ping_fresh = false;
  expect(!mesh_mode_should_use_solo(&input), "missing GCS ping selected solo");
  puts("ok 3 - missing self ping selects mesh");
