...
# Use the large MiDaS v2.1 ONNX model by default for better depth quality.
...
HIGH_ACCURACY_INPUT_SIZE = 384

depth_model_name = os.getenv("DEPTH_MODEL_NAME", HIGH_ACCURACY_MODEL_NAME)
...
if (
    not os.path.exists(depth_model_path)
    and os.path.basename(depth_model_path) == HIGH_ACCURACY_MODEL_NAME
):
    model_dir = os.path.dirname(depth_model_path)
    if model_dir:
        os.makedirs(model_dir, exist_ok=True)
    try:
        print(f"Downloading high-accuracy depth model: {HIGH_ACCURACY_MODEL_NAME}")
        urllib.request.urlretrieve(HIGH_ACCURACY_MODEL_URL, depth_model_path)
        print(f"Saved model to: {depth_model_path}")
    except Exception as exc:
        print(f"Failed to download model from {HIGH_ACCURACY_MODEL_URL}")
...
model_input_dim = int(os.getenv("DEPTH_INPUT_SIZE", str(HIGH_ACCURACY_INPUT_SIZE)))
