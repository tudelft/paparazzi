                       ...
                       size_t len)
{
        ...
        !IS_TIME_EQUAL(gpsdata->fix.time, fix_time))
    {
        ...
        if(strcmp(ac, "NONE") != 0) {
            IvySendMsg("%s TARGET_POS %s %s %d %d %d %f %f %f 0", "0", "0", ac, (int)(gpsdata->fix.latitude * 1e7), (int)(gpsdata->fix.longitude * 1e7), (int)(fix_altitude* 1000), fix_speed, fix_climb, fix_track);
            if (verbose)
