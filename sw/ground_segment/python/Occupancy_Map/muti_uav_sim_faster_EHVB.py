...
# Persistent drift-cone graphics (Placeholders)
...
#     axs_air[i].set_ylim(0, 25) # Fixed Y-range
#     axs_air[i].grid(True)
...
if __name__ == "__main__":
    ...
    for t in range(500):
        ...
        # ------------------------------------------
        # 2 & 3. SKIPPED (Visualization/Weighting)
        #    We don't need global signal maps for the logic anymore.
        #    We calculate visibility on-the-fly per drone below.
        # ------------------------------------------
        ...
        for d_idx in range(num_drones):
            ...
            # the drone will only accelerate at 2m/s^2 to get there.
            max_accel = 2.0 
        ...
        # 8. Energy Update
        # ============================
        # 300 Wh battery = 1,080,000 Joules
        # battery_capacity_J = 18000.0 * 60 
        # Scaling factor: 1 Joule = X percent
        # E_pct_scale = 100.0 / battery_capacity_J

        # for d_idx in range(num_drones):
        #     vx, vy, vz = drone_vels[d_idx]
        #     v_air_mag = np.linalg.norm(np.array([vx, vy]) - v_wind[:2])
            
        #     # Base Power
        #     P_inst = get_vsqp_power(v_air_mag)
            
        #     # Vertical Penalty
        #     if vz > 0.1: P_inst += 1000.0 * vz
        #     elif vz < -0.1: P_inst -= 200.0 * abs(vz)
            
        #     # Mode Penalty (Tracking requires agility)
        #     if drone_modes[d_idx] == "track":
        #         P_inst = max(P_inst, 1751.0 * 1.05)
            
        #     P_inst = max(100.0, P_inst) # Min idle power
            
        #     # Subtract Energy
        #     E_rem[d_idx] -= P_inst * dt_step * E_pct_scale
        #     E_rem[d_idx] = max(0.0, E_rem[d_idx])
        ...
        # ============================
        # 10. Graphs Update & Console Log
        # ============================
        
        # 1. ALWAYS Record Data (Physics runs every step)
        # vel_time.append(t)
        # for d_idx in range(num_drones):
        #     vx, vy, vz = drone_vels[d_idx]
        #     vel_history[d_idx] = np.vstack((vel_history[d_idx], [vx, vy, vz]))

        # # 2. CONDITIONAL Rendering (Draw every 5th frame for Speed)
        # VIZ_INTERVAL = 5
        ...
        #     # A. Update Plot Lines
        #     for d_idx in range(num_drones):
        #         times = np.arange(len(vel_history[d_idx]))
        #         vel_lines[d_idx][0].set_data(times, vel_history[d_idx][:, 0]) # vx
        #         vel_lines[d_idx][1].set_data(times, vel_history[d_idx][:, 1]) # vy
        #         vel_lines[d_idx][2].set_data(times, vel_history[d_idx][:, 2]) # vz
        ...
        #     # C. Flush Graph Windows
        #     fig_vel.canvas.draw_idle()
        #     fig_vel.canvas.flush_events()
            
        #     # D. PRINT ALL VELOCITIES (Corrected Loop)
        #     print(f"--- Time t={t} ---")
        #     for d_idx in range(num_drones):
        #         vx, vy, vz = drone_vels[d_idx]
        #         print(f"  UAV{d_idx}: vx={vx:6.2f}, vy={vy:6.2f}, vz={vz:6.2f}")
        ...
        #     for d_idx in range(num_drones):
        #         times = np.arange(len(vel_history[d_idx]))
        #         vel_lines[d_idx][0].set_data(times, vel_history[d_idx][:, 0])
        #         vel_lines[d_idx][1].set_data(times, vel_history[d_idx][:, 1])
        #         vel_lines[d_idx][2].set_data(times, vel_history[d_idx][:, 2])
        ...
        # Small pause for animation smoothness
        # plt.pause(0.0001)
        # Small pause for animation smoothness
        # plt.pause(0.0001)
        ...
        # Airspeed
        # for d_idx in range(num_drones):
        #     v_air_mag = np.linalg.norm(drone_vels[d_idx][:2] - v_wind[:2])
        ...
        # # Rescale Axes
        # for ax in list(axs_vel) + list(axs_air) + [ax_energy]:
        #     ax.relim()
        #     ax.autoscale_view()
        ...
        # (Optional) Print it with your velocity vector for real-time feedback
        # print(f"t={t} | Freq={current_freq:.1f}Hz | UAV0 vx={vx:.2f}...")
