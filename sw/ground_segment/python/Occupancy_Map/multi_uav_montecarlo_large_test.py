...
# Define a square geofence centered at (0,0)
half_s = map_size / 2
softgeo_xy = np.array([
    [-half_s, -half_s],
    [ half_s, -half_s],
...
buffer_val = 30.0
safe_poly = soft_poly.buffer(-buffer_val)

# Grid resolution adjustment for 5000m to maintain performance
# If 10m is too slow for 5000m, consider 20.0
grid_res = 20.0 
...
def run_single_sim(num_drones, num_victims, severity_settings, seed, composition="mixed"):
    ...
    pred_depth = 3
     ...
     # --- Composition-Aware Altitude Assignment ---
    if composition == "all_low":
        nominal_alts = np.full(num_drones, 40.0)
    elif composition == "all_high":
        nominal_alts = np.full(num_drones, 80.0)
    else: # "mixed" (50/50 split)
        n_low = num_drones // 2
        nominal_alts = np.concatenate([np.full(n_low, 40.0), 
                                       np.full(num_drones - n_low, 80.0)])
    ...
    # --- RANDOMIZED CENTER LAUNCH (ENU 0±100, 0±100, Z) ---
    # Apply a small jitter to break symmetry and prevent drones from following each other
    drone_positions = []
    for alt in nominal_alts:
        jx = np.random.uniform(-100.0, 100.0)
        jy = np.random.uniform(-100.0, 100.0)
        drone_positions.append(np.array([jx, jy, alt]))

    uav_nominal_altitudes = nominal_alts.copy()
...
if __name__ == "__main__":
    ...
    # MANUALLY sync results_dir with the map_size defined above
    results_dir = f"mci_results_{map_size}_second_m_final"
    logs_dir = os.path.join(results_dir, "detailed_json_logs2")
    os.makedirs(logs_dir, exist_ok=True)
    summary_csv = os.path.join(results_dir, "mci_lookup_table2.csv")
    # Define swarm sizes for this map scale
    swarm_sizes = [20, 22, 24, 26, 28, 30] if map_size == 2000 else [90, 95, 100, 105, 110]
    victim_scales = [10, 20, 30] if map_size == 2000 else [5, 10, 25, 50, 100, 125, 150]
    compositions = ["all_low", "all_high", "mixed"]
    severities = {"Low": (2.0, 0.06), "Medium": (5.0, 0.18), "High": (10.0, 0.30)}
    
    trials = 10 

    for sev_label, settings in severities.items():
        for n_vics in victim_scales:
            for n_drones in swarm_sizes:
                for comp in compositions:
                    for trial in range(trials):
                        ...
                        # Execute
                        data = run_single_sim(n_drones, n_vics, settings, trial, composition=comp)
                        
                        # Add metadata strings to the data dict before saving JSON
                        data["severity_label"] = sev_label
                        ...
                        df_row = pd.DataFrame([{
                            ...
                            "victims_found": data["confirmed_count"],
                            "met_600s_limit": 1 if (data["time_elapsed"] < 600 and data["confirmed_count"] == n_vics) else 0,
                            "avg_energy_joules": np.mean(data["energy_per_drone"]),
                            "wall_time": time.time() - start_wall
                        }])
                        df_row.to_csv(summary_csv, mode='a', index=False, header=not os.path.exists(summary_csv))
                        
                        # --- Save Detailed JSON ---
                        log_file = os.path.join(logs_dir, f"log_{sev_label}_V{n_vics}_N{n_drones}_{comp}_t{trial}.json")
                        with open(log_file, 'w') as f:
                            json.dump(data, f, indent=4)
                        
                        print(f"[{sev_label}] V:{n_vics} N:{n_drones} | {comp} | Found: {data['confirmed_count']}/{n_vics}")