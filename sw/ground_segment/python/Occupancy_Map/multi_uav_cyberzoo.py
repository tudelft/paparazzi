...
# --- PAPARAZZI BRIDGE SETUP ---
# Ensure Pprzlink is in the path
...
# Scales for fixed-point INS messages
POS_SCALE = 0.0039063
VEL_SCALE = 0.0000019

# CyberZoo Reference (Must match Paparazzi Center)
...
def on_ins(ac_id, msg):
    if msg.name not in ["INS", "INS_EKF2"]:
        return
...
# --- CRITICAL: Scaling for small area ---
grid_resolution = 0.5  # 50cm grid instead of 10m
step_length = 1.0      # 1m look-ahead instead of 40m
v_max = .5            # 0.5m/s max speed (safe for indoors)
...
def expected_detection(path, belief, grid_points, fov_angle, pred_depth=3):
    ...
    for i in range(len(path) - 1):
        ...
        # Sample points along the segment
        seg_len = np.linalg.norm(p1[:2] - p0[:2])
        n_samples = max(2, int(np.ceil(seg_len / 0.5))) # Sample every 50cm
...
def energy_of_path(path):
    ...
    for i in range(len(path) - 1):
        ...
        if dist_xy > 1e-3:
            ...
            vg_allowed = max(.5, min(.5, .5 + v_headwind_comp)) #vmax + headwind
...
def plan_velocity_ipp_3D(drone_pos, drone_vel, belief, grid_points, soft_poly,
                         ...
                         fov_angle=theta_FOV, v_max=.5, n_directions=16,
                         step_length=1.0, altitude_candidates=[2.0, 3.0],
                         E_scale=100.0,    # From Table 5.1
                         lam=0.4,          # From Table 5.1 (Energy Cost Weight)
                         buffer=1.0, pred_depth=1):
    ...
    # 5. Output Velocity Calculation (Wind-Aware)
...
def propagate_belief_vectorized(belief, grid_points, v_drift, dt, sigma=1.0):
    # Vectorized drift
    ...
    # Use clip to prevent out-of-bounds indices
    x_idx = np.clip(np.searchsorted(x_coords, grid_points[:, 0]), 0, len(x_coords)-1)
    y_idx = np.clip(np.searchsorted(y_coords, grid_points[:, 1]), 0, len(y_coords)-1)

    grid_2d = np.full((len(x_coords), len(y_coords)), 0.5)
    grid_2d[x_idx, y_idx] = new_belief
    
    # Sigma must be small for Cyberzoo (0.5 - 1.0)
    grid_2d = gaussian_filter(grid_2d, sigma=sigma)
    return np.clip(grid_2d[x_idx, y_idx], 1e-6, 1.0 - 1e-6)



def tracking_region(t_now, detection_pos, detection_time, v_drift,
                    base_radius=2.0, uncertainty_gain=1.0,
                    cone_angle_deg=90):
    """
...
def local_centroid(belief, grid_points, idx, radius=1.5):
...
# Victim Template
victims_template = np.array([[-4.0,0.0],[-3.5,1.0], [0, 0], [0.5, 0.5]])
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not AC_IDS:
            ...
            UAVS[122]['state']['x'] = 2.0; UAVS[122]['state']['y'] = -3.0; UAVS[122]['state']['z'] = 2.0
        else:
    ...
    if HAS_REAL_VICTIM:
        ...
        for v_id in active_victim_ids:
            ...
            v_pos, _ = sync_drone_state(0, v_id, np.array([-4.0, 0.0, 0.5]))
    ...
    ax_map.set_xlim(x_min - 2, x_max + 2)
    ax_map.set_ylim(y_min - 2, y_max + 2)
    ...
    buffer = 1.0 # safe buffer for polygon
    ...
    COOLDOWN_TIME = 15  # seconds of detection lockout
    cone_artists = []

    # --- Tracking timing parameters ---
    initial_lock_duration = 5.0    # seconds tracking cone center before IPP
    cone_search_timeout   = 15.0   # max time doing IPP inside cone before abort
    HOVER_CONFIRM_TIME = 5.0  # seconds at low altitude before abort if not confirmed
    ...
    # Clear previous lines from plot if any (optional cleanup)
    # ... (Assuming fresh run, existing global setup is fine)

    # -----------------------------
    # C. MAIN LOOP
    # -----------------------------
    try:
        # Increase loop count or make infinite while True
        ...
        for t in range(1000):
            ...
            # =========================================================
            # 0. SYNC STATE FROM PAPARAZZI
            # =========================================================
            for d_idx in range(num_drones):
                # Only sync from external world if we are NOT in internal physics mode
                if not USE_INTERNAL_PHYSICS:
                    ...
                    # drone_vels[d_idx] = real_vel # Optional sync


            # =========================================================
            # 0. SYNC STATE FROM PAPARAZZI (THE MISSING PIECE)
            # =========================================================
            # This updates 'drone_positions' with the REAL coordinates from Ivy.
            # Without this, and without the "+=" line later, the drones won't move on the map.
            # for d_idx in range(num_drones):
            #     ac_id = AC_IDS[d_idx]
                
            #     # Fetch real pos from the global UAVS dict (via helper function)
            #     real_pos, real_vel = sync_drone_state(d_idx, ac_id, drone_positions[d_idx])
                
            #     # Update the variable used by the Planner and the Plotter
            #     drone_positions[d_idx] = real_pos
                
                # Optional: Update velocity for better energy/planning estimates
                # (Use this if you want the planner to know the REAL velocity)
            ...
            # ------------------------------------------
            # 1. Update Victim Position (Real or Virtual)
            # ------------------------------------------
            # Replace the old "victims[:, :2] += ..." line with this block:
            
            if HAS_REAL_VICTIM:
                # If using real drones (AC123, etc.), read their actual GPS position
                for i, v_id in enumerate(active_victim_ids):
                    # Sync state to get current real position
                    vic_pos, _ = sync_drone_state(0, v_id, np.zeros(3))
                    
                    # Update the planner's truth (victims array)
                    victims[i] = vic_pos[:2]
            else:
                # If no real victim connected, continue using virtual drift
                victims[:, :2] += v_drift * dt_step
            # ------------------------------------------
            ...
            for d_idx in range(num_drones):
                ...
                # --- FIX: ALTITUDE GATE (Stops "Red Map" & "Satellite" Bugs) ---
                # 1. If drone is on the ground (< 0.5m), it sees nothing useful.
                # 2. If drone is glitching high (> 6.0m), it sees too much (false positives).
                # skipping the update prevents "garbage" data from corrupting the map.
                if drone_pos[2] < 0.5 or drone_pos[2] > 6.0:
                    detection_event[d_idx] = False # Ensure no fake detection event remains
                    continue
                ...
                # Extract q = p_hit for visible cells
                q = p_hit[vis_mask]
            ...
            for d_idx in eligible_explorers:
                ...
                # Ignore detections if drone is too high (e.g. > 6m).
                # This prevents "Satellite" bugs where a Z-glitch makes the drone see the whole map.
                if drone_positions[d_idx][2] > 5.0:
                    continue
                ...
                # --- FIX 2: Smarter localization ---
                
                # 1. Get data for just the visible cells
                fov_indices = np.where(vis_mask)[0]
                fov_belief = belief[vis_mask]
                fov_coords = grid_points[vis_mask]

                # 2. Check if belief is "flat"
                belief_range = np.max(fov_belief) - np.min(fov_belief)

                if belief_range < 0.05:
                    # Case A: Flat belief. Assume victim is under the drone (Mean).
                    raw_pos = np.mean(fov_coords, axis=0)
                else:
                    # Case B: Strong hotspot. Trust the peak.
                    best_local_idx = np.argmax(fov_belief)
                    raw_pos = fov_coords[best_local_idx]
                
                # 3. Snap to nearest grid point
                dists = np.sum((grid_points - raw_pos)**2, axis=1)
                final_idx = np.argmin(dists)
                detection_pos = grid_points[final_idx].copy()
                
                # Debug print to confirm altitude when detection happens
                print(f"[DEBUG] Detection by UAV{d_idx} at Alt={drone_positions[d_idx][2]:.1f}m. Pos: {detection_pos}")
            ...
            if (not NEW_DETECTION) or detection_uav is None:
                ...
                for tr_exist in track:
                    ...
                    if region_poly_exist.buffer(1.0).contains(det_point):
                        is_duplicate = True
                        break
            ...
            for d_idx in range(num_drones):
                ...
                if drone_modes[d_idx] == "explore":
                    vx_des, vy_des, vz_des, *_ = plan_velocity_ipp_3D(
                        ...
                        step_length=1.0,       # Large steps
                        fov_angle=theta_FOV, v_max=v_max, n_directions=16,
                        altitude_candidates=[2.5, 5.0], pred_depth=pred_depth, E_scale=E_scale,   # Matches Table 5.1
                        lam=0.5,        
                        buffer=buffer
                    )
                    ...
                    if tr["phase"] == "to_detection":
                        ...
                        if dist > 2.0: 
                            ...
                            # Linear braking: 1.5m/s at distance, slowing to 0.5m/s at 2m
                            speed = np.clip((dist - 2.0) * 0.5, .5, v_max)
                        ...
                        if region_poly is not None and not region_poly.is_empty:
                            ...
                            if np.any(in_R_mask):
                                ...
                                if max_p > 0.20 and (drone_positions[d_idx][2] > MIN_ALT + 0.5):
                                    ...
                                    vz_des = np.clip(target_alt - current_alt, -.5, 0.0)
                                ...
                                if z <= MIN_ALT + 0.2:
                                    ...
                                    # FIX: Initialize timer if it doesn't exist yet
                                    if "fail_timer" not in tr:
                                        tr["fail_timer"] = 0.0
                                    ...
                                    if tr["fail_timer"] > 8.0:
                                        ...
                                        vx_des, vy_des, vz_des = 0.0, 0.0, .5
                                        continue
                        ...
                        # BUFFER THE CONE: Allow the drone to be 1m outside without panicking.
                        # This creates a "Soft Wall" so it curves back in smoothly.
                        soft_cone_constraint = region_poly.buffer(1.0) 
                        ...
                        if np.sum(hotspot_mask) > 3:
                            ...
                            # --- GATING / TRUST CHECK ---
                            # Only trust visual targets within 2m of the expected location.
                        ...
                        if dist_err > 0.2:
                            speed = np.clip(dist_err * .5, 0.5, v_max) 
                        ...
                        if current_alt < 5.0 and mean_p < 0.20 and max_p < 0.40:
                        ...
                        if (
                            ...
                            nominal_alt = uav_nominal_altitudes[d_idx]
                            vz_des = np.clip(nominal_alt - current_alt, -.5, .5)
                ...
                # --- Fallback: any other mode → regular IPP ---
                    ...
                    vx_des, vy_des, vz_des, *_ = plan_velocity_ipp_3D(
                        ...
                        fov_angle=theta_FOV, v_max=v_max, n_directions=16, step_length=1.0,
                ...
                if drone_modes[d_idx] == "track":
                    ...
                    # Limit: Max 1 m/s change per second 
            ...
            for d_idx in range(num_drones):
                ...
                if not safe_poly.contains(point_next):
                    ...
                    if norm > 1e-3:
                        ...
                        speed_limit = max(.5, min(.5, v_g_allowed)) 
            ...
            # ------------------------------------------
            # 8. Advance UAV positions & SEND COMMANDS
            # ------------------------------------------
            # for d_idx in range(num_drones):
            #     # Update Sim State
            #     drone_positions[d_idx] += drone_vels[d_idx] * dt_step
                
            #     # --- ADD THIS: SEND TO PAPARAZZI ---
            #     if ivy:
            #         # Map the loop index (0,1,2) to the real AC_ID (e.g., 121, 122)
            #         ac_id = AC_IDS[d_idx] 
            #         vx, vy, vz = drone_vels[d_idx]
                    
            #         # Send the command
            #         send_pprz_velocity(ivy, ac_id, vx, vy, vz)

            
            # ------------------------------------------
            # 8. Send Smoothed Commands to Paparazzi
            # ------------------------------------------
            
            # Initialize history if it doesn't exist yet (safe for loop restarts)
            # if 'last_cmd_vel' not in locals():
            #     last_cmd_vel = [np.zeros(3) for _ in range(num_drones)]
            
            # # Smoothing Factor (0.0 = freezes old command, 1.0 = no smoothing)
            # # 0.3 to 0.5 is usually the "sweet spot" for lagging wifi/sims
            # ALPHA_VEL = 0.4  

            # for d_idx in range(num_drones):
            #     # 1. Get the Raw Desired Velocity (from the Planner)
            #     raw_vel = drone_vels[d_idx] 
                
            #     # 2. Smooth the Command (Low Pass Filter)
            #     # We blend the previous command with the new one to avoid jerky steps
            #     smoothed_vel = (1.0 - ALPHA_VEL) * last_cmd_vel[d_idx] + ALPHA_VEL * raw_vel
                
            #     # 3. Update History & State
            #     last_cmd_vel[d_idx] = smoothed_vel
                
            #     # IMPORTANT: Update 'drone_vels' with the SMOOTHED value
            #     # This ensures your plots (velocity/energy) match what you actually sent to the drone.
            #     drone_vels[d_idx] = smoothed_vel
                
            #     # 4. Send to Paparazzi
            #     if ivy:
            #         ac_id = AC_IDS[d_idx]
            #         # send_pprz_velocity handles the ENU -> NED conversion for you
            #         send_pprz_velocity(ivy, ac_id, smoothed_vel[0], smoothed_vel[1], smoothed_vel[2])

                
    
                # NOTE: We REMOVED the line: drone_positions[d_idx] += ...
                # Because 'sync_drone_state' (at Step 0) now updates the position 
                # from the real telemetry every frame.
            ...
            for d_idx in range(num_drones):
                ...
                # 4. Send to Paparazzi (Only if Ivy exists)
                if ivy:
                    ac_id = AC_IDS[d_idx]
                    send_pprz_velocity(ivy, ac_id, smoothed_vel[0], smoothed_vel[1], smoothed_vel[2])

                # 5. INTERNAL PHYSICS FALLBACK
                # If we are in simulation mode OR ivy failed to load, move the drone manually.
                if USE_INTERNAL_PHYSICS or ivy is None:
                    drone_positions[d_idx] += drone_vels[d_idx] * dt_step

            # ------------------------------------------
            # 8b. Control the Victim Drones (AC 123)
            # ------------------------------------------
            # This sends the drift velocity to the REAL victim drone so it moves physically.
            if HAS_REAL_VICTIM and ivy:
                for v_id in active_victim_ids:
                    # Send constant wind drift velocity (vx, vy) + 0 vertical speed
                    # Make sure the victim drone is in GUIDED mode!
