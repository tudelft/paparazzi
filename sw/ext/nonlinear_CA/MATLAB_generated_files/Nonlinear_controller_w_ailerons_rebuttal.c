 ...
 * File: Nonlinear_controller_w_ailerons_rebuttal.c
 *
...
/* Include Files */
#include "Nonlinear_controller_w_ailerons_rebuttal.h"
 ...
 *                double transition_airspeed
 *                const double modeled_acc_array[6]
 *                double u_out[15]
 *                double residuals[6]
 *                double *elapsed_time
 *                double *N_iterations
 ...
 * Return Type  : void
 */
void Nonlinear_controller_w_ailerons_rebuttal(
    ...
    double *cost_value, double *exitflag)
{
  ...
  int i;
  char c_expl_temp[3];
  if (!isInitialized_Nonlinear_controller_w_ailerons_rebuttal) {
    Nonlinear_controller_w_ailerons_rebuttal_initialize();
  }
  ...
  if (b_desired_motor_value.contents < min_omega) {
    b_desired_motor_value.contents =
        (((Omega_1 + Omega_2) + Omega_3) + Omega_4) / 4.0;
  }
  ...
  for (i = 0; i < 6; i++) {
    dv_global.contents[i] = dv[i] + modeled_acc_array[i];
  }
  /* Pseudo-control hedging:  */
  if (b_V.contents > transition_airspeed) {
    CL_min = b_Cl_alpha.contents * min_alpha;
    max_theta_protection = b_V.contents;
    max_theta_protection =
        CL_min * 0.5 * b_rho.contents * b_S.contents *
        (max_theta_protection * max_theta_protection) *
        cos(min_theta_protection * 3.1415926535897931 / 180.0);
    max_theta_protection = 9.81 - max_theta_protection / b_m.contents;
    CL_min = 0.0;
    if (dv_global.contents[2] >= max_theta_protection) {
      CL_min = dv_global.contents[2] - max_theta_protection;
    }
    dv_global.contents[2] -= CL_min;
  }
  ...
  *cost_value =
      fmincon(&b_expl_temp, u_out, u_min, u_max, exitflag, N_iterations,
              N_evaluation, c_expl_temp, &max_theta_protection, &CL_min,
              &min_theta_protection, &d_expl_temp);
  *elapsed_time = toc();
  min_theta_protection = gain_motor.contents;
  max_theta_protection = gain_el.contents;
  CL_min = gain_az.contents;
  u_out[0] *= min_theta_protection;
  u_out[4] *= max_theta_protection;
  u_out[8] *= CL_min;
  u_out[1] *= min_theta_protection;
  u_out[5] *= max_theta_protection;
  u_out[9] *= CL_min;
  u_out[2] *= min_theta_protection;
  u_out[6] *= max_theta_protection;
  u_out[10] *= CL_min;
  u_out[3] *= min_theta_protection;
  u_out[7] *= max_theta_protection;
  u_out[11] *= CL_min;
...
/*
 * File trailer for Nonlinear_controller_w_ailerons_rebuttal.c
 *
 * [EOF]
 */
