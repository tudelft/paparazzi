  ...
  do {
    ...
    if (solution->state == -5) {
      ...
      if (subProblemChanged) {
        ...
        if (solution->state != -5) {
          exitg1 = 1;
        } else if ((d_xnrm2(nVar, solution->searchDir) <
                    options_StepTolerance) ||
                   (workingset->nActiveConstr >= nVar)) {
          guard2 = true;
        } else {
          minLambda = b_feasibleratiotest(
          ...
          if (updateFval) {
            ...
            if (objective->objtype == 5) {
              if (d_xnrm2(objective->nvar, solution->searchDir) >
      ...
      if (guard1) {
        ...
        if (solution->iterations - solution->iterations / 50 * 50 == 0) {
          solution->maxConstr =
              d_maxConstraintViolation(workingset, solution->xstar);
          ...
          if (minLambda > 1.0E-6) {
            ...
            activeSetChangeID = 0;
            minLambda =
                d_maxConstraintViolation(workingset, solution->searchDir);
        ...
        if (updateFval && (options_ObjectiveLimit > rtMinusInf)) {
          solution->fstar = b_computeFval_ReuseHx(
  ...
  do {
    ...
    if (solution->state == -5) {
      ...
      if (guard1) {
        ...
        if (solution->iterations - solution->iterations / 50 * 50 == 0) {
          solution->maxConstr =
              c_maxConstraintViolation(workingset, solution->xstar);
          minLambda = solution->maxConstr;
          if (objective->objtype == 5) {
            minLambda =
                solution->maxConstr - solution->xstar[objective->nvar - 1];
          ...
          if (minLambda > 1.0E-6) {
            ...
            activeSetChangeID = 0;
            minLambda =
                c_maxConstraintViolation(workingset, solution->searchDir);
            if (minLambda < solution->maxConstr) {
              if (i - 1 >= 0) {
                memcpy(&solution->xstar[0], &solution->searchDir[0],
                       (unsigned int)i * sizeof(double));
