  ...
  if (!dxTooSmall) {
    ...
    if (isFeasible &&
      ...
      if (isFeasible && (TrialState->sqpFval < -1.0E+20)) {
        ...
        if (guard1) {
          if (TrialState->sqpIterations >= max_iterations_outer_loop || toc() >= max_time_optimizer_outer_loop) {
            Flags->done = true;
            TrialState->sqpExitFlag = 0;
          } else if (TrialState->FunctionEvaluations >= max_function_eval_outer_loop ) {
            Flags->done = true;
            TrialState->sqpExitFlag = 0;
          }
        }
