...
#: Which aircraft relay for the others (AT+TYPE=0). Everything else, including
#: the ground station, is a terminal node (AT+TYPE=1).
#:
#: Sizing rationale, from sw/tools/mesh/mesh_link_budget.py over the confirmed
#: 3 km x 3 km operating area with the GCS in a corner:
#:
#:   * the worst geometry in the box is 4.24 km (corner to corner), and every
#:     link closes there with 7.3 dB to spare on top of the 10 dB fade margin,
#:     so the mesh does not need multi-hop to be CONNECTED;
#:   * but that spare does not survive a pessimistic interference budget
#:     (6 dB of raised noise floor, 3 dB for a 60 deg bank, 4 dB of airframe
#:     shadowing), so a single path is not enough to be RELIABLE.
#:
#: One relay takes the modelled outage from 10 % to 1.9 % - a 5x improvement -
#: and, because a 2-hop flood still fits inside the 62.5 ms slot, it costs
#: nothing in position update rate. A second relay would buy another 5x but
#: only by stretching the superframe to 0.75 s, i.e. by giving up a third of
#: the update rate.
#:
#: The relay should be whichever airframe is expected to fly most centrally and
#: stay aloft longest. If it lands, the mesh degrades gracefully to flat: still
#: fully connected at 4.24 km, just without the diverse second path.
RELAY_AC_IDS = (125,)
...
#:
#: It is nevertheless provisioned as a TERMINAL node, exactly like the aircraft
#: that are not the designated relay. Relaying does not help it RECEIVE - every
#: drone is already inside its direct range across the whole box - and making it
#: route would add a hop to every frame in the network for nothing. Its rare
#: commands are unslotted and simply contend, which is what "sparingly" affords.
GCS_IS_ROUTING = False
...
DEFAULTS = {
    ...
    "node_type": 1,        # 0 = routing node, 1 = terminal node
...
def build_profile(ac_id: int, args: argparse.Namespace) -> List[Step]:
    ...
    steps += [
        ...
        Step(f"AT+FILTER_TIME={args.filter_time}",
             "duplicate suppression window. 3 s is safe here because each node "
             "originates only once per 4 s superframe. Takes no <save> argument"),
    ]
...
def role_of(ac_id: int, args: argparse.Namespace) -> Tuple[str, int]:
    ...
    if ac_id in args.relay_ids:
        return ("GCS, routing" if ac_id == 0 else "relay, routing"), 0
    if ac_id == 0:
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    ap.add_argument("--relay-ids", default=",".join(str(i) for i in RELAY_AC_IDS),
                    help="comma separated AC_IDs to provision as routing nodes. "
                         "AC_IDs are arbitrary, so the default only matches this "
                         "fleet; give the real list when it differs, or no node "
                         "will relay and the mesh loses its second path")
    ap.add_argument("--node-type", type=int, default=None,
                    choices=(0, 1),
                    help="override the role table: 0=routing node 1=terminal node. "
                         "By default the GCS and the aircraft in RELAY_AC_IDS are "
                         "routing nodes and everything else is a terminal node")
    ...
    ap.add_argument("--nb-slots", type=int, default=8,
                    help="must match MESH_TDMA_NB_SLOTS in traffic_info.h")
    ...
    try:
        ...
        ap.error("--relay-ids must be a comma separated list of integers")
