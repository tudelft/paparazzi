...
class Bebop(ParrotUtils):
    ...
    def write_to_config(self, name, value):
        if self.read_from_config(name) == 'Unknown':
            ...
            self.execute_command(r'sed -i "s/\(' + name + r' *= *\).*/\\1' + value + r'/g" ' + self.config_file)
    ...
    def bebop_install_scripts(self):
        ...
        if self.check_connect2hub():
            self.execute_command(r"sed -i 's|connect2hub|pprzstarter|' /etc/init.d/rcS")
            self.execute_command("rm /data/ftp/internal_000/scripts/connect2hub")
        else:
            self.execute_command(r"sed -i 's|^exit 0|/data/ftp/internal_000/scripts/pprzstarter \& exit 0|' /etc/init.d/rcS")
    ...
    def bebop_uninstall_scripts(self):
        ...
        self.execute_command("mount -o remount,rw /")
        self.execute_command(r"sed -i 's|^/data/ftp/internal_000/scripts/pprzstarter \& exit 0|exit 0|' /etc/init.d/rcS")
        self.execute_command("chmod a+x /etc/init.d/rcS")
