...
#warning "GUIDANCE_INDI_RC_DEBUG lets you control the accelerations via RC, but disables autonomous flight!"
  ...
  float pitch_cmd = (guidance_euler_cmd.theta >= 0.f) ? (guidance_euler_cmd.theta * MAX_PPRZ / fabs(RadOfDeg(guidance_indi_max_pitch))) : (guidance_euler_cmd.theta * MAX_PPRZ / fabs(RadOfDeg(guidance_indi_min_pitch)));
