...
#define CA_N_V_HYBRID 3
#define CA_N_U_HYBRID 4

#ifndef CA_N_V_HYBRID
#error CA_N_V needs to be defined!
#endif

#ifndef CA_N_U_HYBRID
#error CA_N_U needs to be defined!
#endif

#define CA_N_C_HYBRID  (CA_N_U_HYBRID+CA_N_V_HYBRID)
 ...
 */
void qr_solve_wrapper_hybrid(int m, int n, float** A, float* b, float* x) {
  float in[m * n];
 ...
 */
int wls_alloc_hybrid(float* u, float* v, float* umin, float* umax, float** B,
  ...
  int n_v = CA_N_V_HYBRID;

  float A[CA_N_C_HYBRID][CA_N_U_HYBRID];
  float A_free[CA_N_C_HYBRID][CA_N_U_HYBRID];

  // Create a pointer array to the rows of A_free
  // such that we can pass it to a function
  float * A_free_ptr[CA_N_C_HYBRID];
  for(int i = 0; i < n_c; i++)
    A_free_ptr[i] = A_free[i];

  float b[CA_N_C_HYBRID];
  float d[CA_N_C_HYBRID];

  int free_index[CA_N_U_HYBRID];
  int free_index_lookup[CA_N_U_HYBRID];
  int n_free = 0;
  int free_chk = -1;

  int iter = 0;
  float p_free[CA_N_U_HYBRID];
  float p[CA_N_U_HYBRID];
  float u_opt[CA_N_U_HYBRID];
  int infeasible_index[CA_N_U_HYBRID] UNUSED;
  int n_infeasible = 0;
  float lambda[CA_N_U_HYBRID];
  float W[CA_N_U_HYBRID];
  ...
  while (iter++ < imax) {
    ...
    if (n_free) {
      ...
      qr_solve_wrapper_hybrid(n_c, n_free, A_free_ptr, d, p_free);
