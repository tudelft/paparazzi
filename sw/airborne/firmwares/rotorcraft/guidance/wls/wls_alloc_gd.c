 ...
 */
void qr_solve_wrapper_gd(int m, int n, float** A, float* b, float* x) {
  float in[m * n];
...
float wls_alloc_gd(float* u, float* v, float* umin, float* umax, float** B,
...
//    memset(W, 0, n_u * sizeof(float));
    ...
    while (iter++ < imax) {
        ...
        if (n_free) {
            ...
            qr_solve_wrapper_gd(n_c, n_free, A_free_ptr, d, p_free);
        }
...
float wls_alloc_gd_reduced(float* u, float* v, float* umin, float* umax, float** B,
                   float* u_guess, float* W_init, float* Wv, float* Wu, float* up,
                   float gamma_sq, int imax) {
    // allocate variables, use defaults where parameters are set to 0
    if(!gamma_sq) gamma_sq = 100000;
