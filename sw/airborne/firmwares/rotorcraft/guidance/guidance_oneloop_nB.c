...
/**
 * @file firmwares/rotorcraft/guidance/guidance_oneloop_nB.c
 *
 * A dummy guidance module to run the oneloop_nB controller
 *
 */

#include "firmwares/rotorcraft/guidance/guidance_h.h"
#include "firmwares/rotorcraft/guidance/guidance_v.h"
#include "firmwares/rotorcraft/guidance/guidance_oneloop_nB.h"
#include "firmwares/rotorcraft/oneloop/oneloop_nB.h"
struct ThrustSetpoint thrust_sp;

void guidance_h_run_enter(void)
{
  //oneloop_nB_enter(false, CTRL_ANDI);
}
...
static enum GuidanceOneloop_VMode _v_mode = GUIDANCE_ONELOOP_NB_V_POS;

struct StabilizationSetpoint guidance_h_run_pos(bool in_flight, struct HorizontalGuidance *gh)
{
  return guidance_oneloop_nB_run_mode(in_flight, gh, _gv, GUIDANCE_ONELOOP_NB_H_POS, _v_mode);
}

struct StabilizationSetpoint guidance_h_run_speed(bool in_flight, struct HorizontalGuidance *gh)
{
  return guidance_oneloop_nB_run_mode(in_flight, gh, _gv, GUIDANCE_ONELOOP_NB_H_SPEED, _v_mode);
}

struct StabilizationSetpoint guidance_h_run_accel(bool in_flight, struct HorizontalGuidance *gh)
{
  return guidance_oneloop_nB_run_mode(in_flight, gh, _gv, GUIDANCE_ONELOOP_NB_H_ACCEL, _v_mode);
}

struct ThrustSetpoint guidance_v_run_pos(bool in_flight UNUSED, struct VerticalGuidance *gv)
{
  _gv = gv;
  _v_mode = GUIDANCE_ONELOOP_NB_V_POS;
...
{
  _gv = gv;
  _v_mode = GUIDANCE_ONELOOP_NB_V_SPEED;
  return thrust_sp; // nothing to do
}
...
{
  _gv = gv;
  _v_mode = GUIDANCE_ONELOOP_NB_V_ACCEL;  
  return thrust_sp;; // nothing to do
}

struct StabilizationSetpoint guidance_oneloop_nB_run_mode(bool in_flight, struct HorizontalGuidance *gh, struct VerticalGuidance *gv, enum GuidanceOneloop_HMode h_mode, enum GuidanceOneloop_VMode v_mode)
{
  struct FloatVect3 PSA_des    = { 0 };
  //int    rm_order_h = 3;
  //int    rm_order_v = 3;
  // Oneloop controller wants desired targets and handles reference generation internally
  if (h_mode == GUIDANCE_ONELOOP_NB_H_POS) {
    PSA_des.x   = POS_FLOAT_OF_BFP(gh->sp.pos.x);
    PSA_des.y   = POS_FLOAT_OF_BFP(gh->sp.pos.y);
    //rm_order_h  = 3;
  }
  else if (h_mode == GUIDANCE_ONELOOP_NB_H_SPEED) {
  ...
  if (v_mode == GUIDANCE_ONELOOP_NB_V_POS){
    PSA_des.z   = POS_FLOAT_OF_BFP(gv->z_sp);
    //rm_order_v  = 3;
  }
  else if (v_mode == GUIDANCE_ONELOOP_NB_V_SPEED) {
    PSA_des.z   = SPEED_FLOAT_OF_BFP(gv->zd_sp);
    //rm_order_v  = 2;
  }
  else { // H_ACCEL
  ...
  oneloop_nB.half_loop = false;
  oneloop_nB_run(in_flight, oneloop_nB.half_loop, PSA_des);
