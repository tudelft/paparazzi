...
#ifndef STABILIZATION_INDI_MAX_OUTPUT
#define STABILIZATION_INDI_MAX_OUTPUT 1.0
#endif
...
float indi_max_output = STABILIZATION_INDI_MAX_OUTPUT;
...
void stabilization_indi_rate_run(struct FloatRates rate_sp, bool in_flight)
{
  ...
  for (i = 0; i < INDI_NUM_ACT; i++) {
    du_min[i] = -MAX_PPRZ*indi_max_output * act_is_servo[i] - use_increment*actuator_state_filt_vect[i];
    du_max[i] = MAX_PPRZ*indi_max_output - use_increment*actuator_state_filt_vect[i];
  ...
  for (i = 0; i < INDI_NUM_ACT; i++) {
    if (act_is_servo[i]) {
      ...
      if (autopilot_get_motors_on()) {
        Bound(indi_u[i], 0, indi_max_output*MAX_PPRZ);
      }
      else {
        indi_u[i] = -MAX_PPRZ;
      }
