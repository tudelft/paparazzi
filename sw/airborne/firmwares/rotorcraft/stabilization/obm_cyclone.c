                         ...
                         const float coeff[22], float F_obm_moments[3])
{
  ...
  F_obm_moments[0] = ((coeff[19] * (u[2] - u[3]) + coeff[18] * t8 * body_vel[1]) -
                      coeff[20] * t8 * rates[2]) -
                     rates[1] * coeff[21] * rates[2];
  F_obm_moments[1] = ((((coeff[15] * (t2 + t3) + coeff[14] * (u[2] + u[3])) +
                        coeff[13] * t8 * body_vel[0]) -
                       coeff[12] * t8 * body_vel[2]) -
                      rates[0] * coeff[17] * rates[2]) -
                     coeff[16] * t8 * body_vel[2] * (u[0] + u[1]);
  F_obm_moments[2] = ((-coeff[8] * (t2 - t3) - coeff[10] * (rates[2] * rates[2])) -
                     ...
                     const float coeff[22], float F_stb_x[4])
{
  ...
  F_stb_x[0] =
      -(((((((((-accel_body[1] * coeff[18] * t9 - accel_body[0] * coeff[18] * vel_body[0] * vel_body[1]) -
  ...
  F_stb_x[2] =
      ((((accel_body[2] * (coeff[9] * t12 * t15 + coeff[9] * t10 * t12 / et1_tmp) -
          rates[0] * coeff[11] * ang_accel[1]) -
         rates[1] * coeff[11] * ang_accel[0]) -
        rates[2] * coeff[10] * ang_accel[2] * 2.0F) +
