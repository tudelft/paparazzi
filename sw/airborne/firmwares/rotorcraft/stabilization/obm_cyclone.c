...
static void cyclone_obm_forces(const float body_vel[3], const float u[4],
                        const float coeff[23], float F_obm_forces[3])
{
  float t2;
  float t7;
  /* CYCLONE_OBM_FORCES */
  /*     F_obm_forces = CYCLONE_OBM_FORCES(IN1,IN2,IN3,IN4,IN5) */
  ...
  F_obm_forces[0] =
      (((coeff[3] * t2 + coeff[7] * (u[0] * u[2] + u[1] * u[3])) +
...
static void cyclone_obm_moments(const float ang_rates[3],
                         const float body_vel[3], const float u[4],
                         const float coeff[23], float F_obm_moments[3])
{
  float t10;
  float t2;
  float t3;
  float t8;
  /* CYCLONE_OBM_MOMENTS */
  /*     F_obm_moments = CYCLONE_OBM_MOMENTS(IN1,IN2,IN3,IN4,IN5) */
  /*     This function was generated by the Symbolic Math Toolbox version 24.2.
   */
  /*     2025-11-26 11:05:08 */
  t2 = u[0] * u[2];
  t3 = u[1] * u[3];
  t8 = u[2] - u[3];
  t10 = sqrtf((body_vel[0] * body_vel[0] + body_vel[1] * body_vel[1]) + body_vel[2] * body_vel[2]);
  F_obm_moments[0] =
      ((coeff[20] * t8 + coeff[19] * t10 * body_vel[1]) - coeff[21] * t10 * ang_rates[2]) -
      ang_rates[1] * coeff[22] * ang_rates[2];
  F_obm_moments[1] = ((((coeff[16] * (t2 + t3) + coeff[18] * (ang_rates[0] - ang_rates[2])) +
                        coeff[15] * (u[2] + u[3])) +
                       coeff[13] * t10 * body_vel[0]) -
                      coeff[12] * t10 * body_vel[2]) -
                     coeff[17] * t10 * body_vel[2] * (u[0] + u[1]);
  F_obm_moments[2] =
      ((-coeff[8] * t8 - coeff[9] * (t2 - t3)) - ang_rates[0] * coeff[11] * ang_rates[1]) +
      coeff[10] * t10 * body_vel[2] * (u[0] - u[1]);
}
