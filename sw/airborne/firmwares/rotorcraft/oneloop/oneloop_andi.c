...
#ifndef ONELOOP_ANDI_SIMPLE_QUAD
...
bool          drop_pitch            = false;                                      // Drop the control of the Pitch axis
bool          drop_aN               = false;                                      // Drop the control of the aN axis
bool          drop_aE               = false;                                      // Drop the control of the aE axis
...
float   t_0_chirp           = 0.0;
float   f0_chirp            = 0.2;                // Start Frequency [Hz]
float   f1_chirp            = 0.2;                // End Frequency [Hz]
float   t_chirp             = 0.65 ;              // Time length of the chirp [s]
float   A_chirp             = 250.0;              // Amplitude of the chirp
int8_t  chirp_axis          = 6   ;               // Axis on which the chirp is applied
...
void  oneloop_axis_effectiveness_calc(void); // FIXME check if used
void  oneloop_andi_bound_disturbance(void); // FIXME add flag to turn on and off
...
void oneloop_andi_RM(bool half_loop, struct FloatVect3 PSA_des, int rm_order_h, int rm_order_v, bool in_flight_oneloop)
{
  ...
  if (half_loop)
  {
    ...
    if (fabs(delta_psi_rad) > RadOfDeg(30.0))
    { // If difference is bigger than 10 deg do not further increment desired
  ...
  else
  {
    ...
    if (multiple_chirp_run && !chirp_on)
    {
      if (chirp_n_counter < chirp_n_repeat)
      {
        ...
        if (fabsf(oneloop_andi.sta_state.att_2d[idx_acc]) < RadOfDeg(trigger_acc) && delay_counter >= 2500 && fabsf(oneloop_andi.sta_state.att_d[idx_acc]) < RadOfDeg(5.0))
        {
          firing_counter++;
        }
        else
...
#define USE_PID_OUTERLOOP
  ...
  float y_4d_att[3];
  float att_des_4_ec[3] = {eulers_zxy_des.phi, eulers_zxy_des.theta, eulers_zxy_des.psi};
  if (oneloop_andi.ctrl_type == CTRL_ANDI)
  {
    ...
    ec_3rd_att(y_4d_att, att_des_4_ec, oneloop_andi.sta_ref.att, oneloop_andi.sta_ref.att_d, oneloop_andi.sta_ref.att_2d, oneloop_andi.sta_ref.att_3d, oneloop_andi.sta_state.att, oneloop_andi.sta_state.att_d, oneloop_andi.sta_state.att_2d, k_att_e.k1, k_att_e.k2, k_att_e.k3, sta_bounds, temp_dist_bound_sta);
  }
  else if (oneloop_andi.ctrl_type == CTRL_INDI)
  {
    ...
    ec_3rd_att(y_4d_att, att_des_4_ec, oneloop_andi.sta_ref.att, oneloop_andi.sta_ref.att_d, oneloop_andi.sta_ref.att_2d, dummy0, oneloop_andi.sta_state.att, oneloop_andi.sta_state.att_d, oneloop_andi.sta_state.att_2d, k_att_e_indi.k1, k_att_e_indi.k2, k_att_e_indi.k3, sta_bounds, temp_dist_bound_sta_INDI);
  }
  ...
  //   for(i=0; i<4; i++){
  //     if (i==chirp_axis){
  //       commands[i] = radio_control_get(RADIO_THROTTLE);
  //     }
  //     commands[i] = 0.0;
  //   }
  //&& radio_control_get(RADIO_THROTTLE)>200
  // commands[COMMAND_MOTOR_RIGHT] = 0.0;//radio_control_get(RADIO_THROTTLE);
  // commands[COMMAND_MOTOR_LEFT]  = 0.0;//radio_control_get(RADIO_THROTTLE);
  // commands[COMMAND_MOTOR_FRONT] = 0.0;//radio_control_get(RADIO_THROTTLE);
  // commands[COMMAND_MOTOR_BACK]  = 0.0;//radio_control_get(RADIO_THROTTLE);
  //  }
...
void chirp_pos(float time_elapsed, float f0, float f1, float t_chirp, float A, int8_t n, float psi, float p_ref[], float v_ref[], float a_ref[], float j_ref[], float p_ref_0[])
{
  ...
  case 5:
    ...
    NormRadAngle(psi_des_rad);
    // Set the EC to track a 0 acceleration in Z
    p_ref[2] = oneloop_andi.gui_state.pos[2];
    v_ref[2] = oneloop_andi.gui_state.vel[2];
    a_ref[2] = 0.0;
    drop_aN = true;
    drop_aE = true;
  ...
  case 6:
    ...
    if (time_elapsed < (0.5))
    {
      oneloop_andi.sta_ref.att_2d[0] = 0.0;
    }
    else
    {
