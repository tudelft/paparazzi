...
{
  float u_min[INDI_NUM_ACT] = { -0.785000,-0.244003,-11.409039,0.000, };
  float u_max[INDI_NUM_ACT] = {0.785000,0.243997,1.646960,8.174995, };
 // v = 0.000021,-0.000001,-2.224771 
//umin = -0.785000,-0.244003,-11.409039,0.000000 
//umax = 0.785000,0.243997,1.646960,8.174995 
//Wv = 10.000000,10.000000,1.000000 
//Wu = 104.139999,234.240005,6.260000,1.000000 
  float up[INDI_NUM_ACT] = {-0.000000,-0.000003,-11.409039,0.000000 };
  float g1g2[INDI_OUTPUTS][INDI_NUM_ACT] = {
    {     0.013774, -9.809911, -0.004028, 0.999991},
    { 9.809991, 0.013769, 0.000125, -0.001404},
    {  0.001170, 0.039518, -0.999992, -0.004028},
  };

  //State prioritization {W Roll, W pitch, W yaw, TOTAL THRUST}
  static float Wv[INDI_OUTPUTS] = {10, 10, 1};
  static float Wu[INDI_NUM_ACT] = {104.139999,234.240005,6.260000,1.000000};

  // The control objective in array format
  float indi_v[INDI_OUTPUTS] = {0.000021,-0.000001,-2.224771};
  ...
  int num_iter =
    wls_alloc(indi_du, indi_v, u_min, u_max, Bwls, 0, 0, Wv, Wu, up, 10000, 10);
...
void test_overdetermined(void)
{
  ...
  // float u_c[INDI_NUM_ACT] = {4614, 4210, 4210, 4614};
  ...
  // };
  // //State prioritization {W Roll, W pitch, W yaw, TOTAL THRUST}
  // static float Wv[INDI_OUTPUTS] = {100, 100, 1};

  // // The control objective in array format
  // //float indi_v[INDI_OUTPUTS] = {240,  -240.5658,    600.0,    1.8532};
  // float indi_v[INDI_OUTPUTS] = {240,  -240.5658,    600.0};
  // float indi_du[INDI_NUM_ACT];
  ...
  // // WLS Control Allocator
  // //int num_iter = wls_alloc(indi_du, indi_v, du_min, du_max, Bwls, 0, 0, Wv, 0, u_p, 0, 10);
