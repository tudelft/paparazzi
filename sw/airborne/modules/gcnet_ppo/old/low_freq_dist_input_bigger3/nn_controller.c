...
const float gate_pos_rel[NUM_GATES][3] = {
    {3.4969110629390343e-07, 4.0, 0.0},
    {3.0, 4.0, 0.0},
    {0.0, -4.0, -1.0},
    {2.999999761581421, -4.000000476837158, 1.0},
    {3.4969110629390343e-07, 4.0, 0.0},
    {3.0, 4.0, 0.0},
    {0.0, -4.0, -1.0},
    {2.999999761581421, -4.000000476837158, 1.0},
...
void nn_control(const float world_state[16], const float disturbances[4], float rpms[4]) {
    ...
    for (int i = 0; i < GATES_AHEAD; i++) {
        ...
        // loop back to the first gate if we reach the end
        if (index >= NUM_GATES) {index -= NUM_GATES;}
    ...
    nn_input[16+4*GATES_AHEAD]   = (disturbances[0] - Mx_mean) / Mx_std;
    nn_input[16+4*GATES_AHEAD+1] = (disturbances[1] - My_mean) / My_std;
    nn_input[16+4*GATES_AHEAD+2] = (disturbances[2] - Mz_mean) / Mz_std;
    nn_input[16+4*GATES_AHEAD+3] = (disturbances[3] - Fz_mean) / Fz_std;
