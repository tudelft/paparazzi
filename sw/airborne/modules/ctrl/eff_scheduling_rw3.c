...
#ifndef ROT_MECH_IDX
#error ctrl_eff_sched_rot_wing requires a servo named ROT_MECH_IDX
#endif

#ifndef ROT_WING_EFF_SCHED_IXX_BODY
#error "NO ROT_WING_EFF_SCHED_IXX defined"
#endif

#ifndef ROT_WING_EFF_SCHED_IYY_BODY
#error "NO ROT_WING_EFF_SCHED_IYY defined"
#endif
...
static void wing_position_cb(uint8_t sender_id UNUSED, struct act_feedback_t *pos_msg, uint8_t num_act)
{
  for (int i=0; i<num_act; i++){
    if (pos_msg[i].set.position && (pos_msg[i].idx == ROT_MECH_IDX))
    {
...
void eff_scheduling_rot_wing_init(void)
{
  ...
  eff_sched_var.Iyy               = eff_sched_p.Iyy_body;
...
void eff_scheduling_rot_wing_update_cmd(void)
{
  ...
  eff_sched_var.cmd_elevator      = actuators_pprz[EL_IDX];
  eff_sched_var.cmd_pusher_scaled = actuators_pprz[MP_IDX] * 0.000853229; // Scaled with 8181 / 9600 / 1000
  eff_sched_var.cmd_T_mean_scaled = (actuators_pprz[MF_IDX] + actuators_pprz[MR_IDX] + actuators_pprz[MB_IDX] + actuators_pprz[ML_IDX]) / 4. * 0.000853229; // Scaled with 8181 / 9600 / 1000
}
