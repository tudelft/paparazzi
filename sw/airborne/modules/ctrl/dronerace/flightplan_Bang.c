...
{0, 2.0, 0.0,-1.75,0.0,0.2,GATE,BANGBANG},
};
...
static void update_gate_setpoints(void){
    ...
    #ifdef LOG
        fprintf(fp_logger_t,"%f, %d, %d, %d, %f, %f, %f\n",get_sys_time_float(),dr_bang.gate_nr,dr_bang.controller_type,dr_bang.gate_x,dr_bang.gate_y,dr_bang.gate_z);
    #endif
}
...
void flightplan_run(void){
    ...
    if(dist2gate<1 && abs(error_speed)<1.5){
