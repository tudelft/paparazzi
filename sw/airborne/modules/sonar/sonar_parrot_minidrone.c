 ...
 *  @brief Parrot Minidrones driver for the build in sonar
 *
 *  This file contains the driver for the default sonar ranging sensor availabe on all Parrot Minidrones
 */
...
{
	mode = 0; // Initial mode is high max altitude, most common usecase scenario
	pulse_transition_counter = 0;

	sonar_parrot_minidrone.meas = 0;
	sonar_parrot_minidrone.offset = SONAR_OFFSET;
...
void *sonar_parrot_minidrone_read(void *data __attribute__((unused)))
{
	...
	//*********** Basic things ***********
	struct ultra_sound_context ctx; // = { 0 }; //FIXME: Maybe enable? then there is no need for memset...

	long int nb_acquisition = NB_ACQUISITION_DEFAULT;
	uint8_t SPI_pattern = SPI_PATTERN_DEFAULT;
	uint8_t nb_pulses = NB_PULSES_DEFAULT;
	uint16_t nb_samples = NB_SAMPLES_DEFAULT;
	...
	// ctx.nb_samples_by_acquisition_adc = HAL_P6I_ULTRASOUND_FREQUENCY_ADC/FREQUENCY_WANTED; //TODO: make it work
	...
	if (verbose)
	{
		printf(". Opened sensor successfully\n");
	}
	...
	{
		printf(".. Setting up USND_SETSIZE %d\n", ctx.adc_raw_data_size);
	}
	...
	{
		printf(".. Setting up USND_SPI_LEN %d\n", ctx.nb_pulses);
	}
	...
	{
		printf(".. Setting up USND_SPI_DAT\n");
	}

	if (verbose && ret == 0)
	{
		printf(". Setup of Ultrasonic sensor successfully\n");
	}
	...
	while (true)
	{
		...
		if (verbose && ret==0) {
		   printf(". Got data successfully ADC size: %lu a sampled value %lu %lu\n", (unsigned long)ctx.adc_raw_data_size, (unsigned long)ctx.adc_raw_data[0],(unsigned long)ctx.adc_raw_data[1]);
	 }

		// TODO: Now here add all the nifty code already in Bebop Sonar Driver if basic meas and deistance are working 100%

		uint32_t tempy = 45678; //TODO: For debugging only, remove later
		if (ret == 0)
		{
			tempy = (uint32_t)&ctx.adc_raw_data[0]; //TODO: For debugging only, remove later
			//sonar_parrot_minidrone.meas = (uint16_t)rand() % 100; //DEBUG only just to check if we have update data in our messages a quick n dirty way
			sonar_parrot_minidrone.meas = (uint16_t)(tempy / 10000);//Note that with cast n stuff the UINT16_MAX
			sonar_parrot_minidrone.distance = (float)(tempy);				 //TODO: for DEBUG only remove later if readings work...
			if (verbose) { printf(". Got data successfully ADC size: %lu a sampled value %lu\n", (unsigned long)ctx.adc_raw_data_size, (unsigned long)ctx.adc_raw_data[0]); }
		}
		else
		{
			break;
		...
		//    if (peak_distance > 0) {
		// Send ABI message
		//TODO: activate and check (and fix if it does not work)
		...
		//sonar_parrot_minidrone_downlink(); //Enable later if all works
		#endif
		//     }
	}
	//*********** END  ***********
