...
def rectangle(parent, horizontal, vertical, width, height, fill, stroke="none"):
    element(parent, "rect", x=horizontal, y=vertical, width=width, height=height,
            fill=fill, stroke=stroke, stroke_width=2)


def line(parent, start, end, color=TEAL, arrow=False):
...
def canvas(title, subtitle, height=620):
    ...
    element(marker, "path", d="M0,0 L0,6 L7,3 Z", fill=TEAL)
    rectangle(root, 0, 0, 880, height, "#f4f8fa")
    rectangle(root, 32, 32, 6, 54, TEAL)
    text(root, 54, 54, title, 26, weight="bold")
    text(root, 54, 83, subtitle, 16)
    return root


def board(parent, left, top, pitch, corners=False):
    rectangle(parent, left - pitch / 2, top - pitch / 2, pitch * 9, pitch * 8, "#c6d2d7", INK)
    for row in range(7):
        for column in range(8):
            rectangle(parent, left + column * pitch, top + row * pitch, pitch, pitch,
                      "#e9eef0" if (row + column) % 2 else "#29434c")
    if corners:
        for row in range(1, 7):
            for column in range(1, 8):
                element(parent, "circle", cx=left + column * pitch, cy=top + row * pitch,
                        r=4, fill="#ffba55", stroke="#753000", stroke_width=1)


def save(root, name):
    ET.indent(root)
    ET.ElementTree(root).write(ROOT / f"lwir-calibration-{name}.svg", encoding="utf-8", xml_declaration=True)
...
def target():
    ...
    rectangle(root, 530, 237, 24, 24, "#29434c")
    text(root, 569, 256, "Thin matte tape", 19)
    rectangle(root, 530, 277, 24, 24, "#e9eef0", INK)
    text(root, 569, 296, "Smooth foil", 19)
    text(root, 530, 347, "Dots show crossings.", 18)
    text(root, 530, 375, "Do not add dots to the target.", 17)
    text(root, 530, 420, "Flat backing, clean edges,", 18)
    text(root, 530, 447, "uniform border, no overlaps.", 18)
    line(root, (80, 530), (320, 530))
    line(root, (80, 518), (80, 542))
    line(root, (320, 518), (320, 542))
    text(root, 80, 570, "5 pitches = 250 mm gives 50 mm per square", 19)
    text(root, 80, 599, "Example only. Measure both directions on the finished board.", 16)
    save(root, "target")
...
def coverage():
    ...
    for index, (label, points) in enumerate(zip(labels, shapes)):
        ...
        rectangle(group, 0, 0, 240, 145, "white", "#8ba5af")
        line(group, (120, 0), (120, 145), "#dce6e8")
        line(group, (0, 72), (240, 72), "#dce6e8")
        element(group, "polygon", points=points, fill="#c1e6de", stroke=TEAL, stroke_width=3)
        text(root, left, top + 187, "Target footprint in sensor", 14)
    text(root, 40, 634, "25-35 varied fitting views", 24, TEAL, "bold")
    text(root, 40, 673, "+ 5-8 new views kept only for checking", 23, AMBER, "bold")
    text(root, 40, 714, "Include all edges and corners; these six poses are examples, not the full set.", 17)
    save(root, "coverage")
...
def mount():
    ...
    line(root, (480, 420), (480, 158), arrow=True)
    text(root, 505, 172, "FORWARD / nose", 22, TEAL, "bold")
    line(root, (450, 530), (745, 530), arrow=True)
    text(root, 545, 567, "BODY RIGHT", 22, TEAL, "bold")
    for label, horizontal, vertical in (("0", 152, 262), ("1", 194, 262), ("7", 152, 304)):
        element(root, "circle", cx=horizontal, cy=vertical, r=15, fill="#fff0d6", stroke=AMBER, stroke_width=2)
        text(root, horizontal - 6, vertical + 6, label, 18, AMBER, "bold")
    text(root, 90, 162, "Near nose", 18)
    text(root, 92, 564, "Rows increase toward tail", 18)
    ...
    text(root, 535, 386, "DOWN goes into this page.", 17)
    text(root, 535, 425, "Mark zero on the border,", 17)
    text(root, 535, 452, "not on a crossing.", 17)
    text(root, 40, 623, "Confirm labels on the saved preview before trusting the rotation.", 21, AMBER, "bold")
    text(root, 40, 658, "This ordering applies to the 7-column inner-corner pattern shown here.", 17)
    save(root, "mount")
