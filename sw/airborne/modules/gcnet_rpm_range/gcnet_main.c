...
/**
 * @file "modules/gcnet_rpm_range/gcnet_main.c"
...
#include "modules/gcnet_rpm_range/gcnet_main.h"
#include "state.h"
...
/** RPM peak tracker seb */
#include "modules/rpm_peak_tracker/rpm_peak_tracker.h"
...
*/
	...
	// 	// More waypoints
		...
		else if (WP_EQUALS(WP_GOAL, WP_WP5)) {
			psi_ref = atan2(waypoints[WP_WP1].enu_f.x-waypoints[WP_WP4].enu_f.x, waypoints[WP_WP1].enu_f.y-waypoints[WP_WP4].enu_f.y);
			
			// set next waypoint once drone passes the plane perpendicular to the waypoint (psi_ref + pi/4 direction)
			//float normal_x = cos(psi_ref + M_PI/4);
			//float normal_y = sin(psi_ref + M_PI/4);		
			//if (-delta_pos_ned.x*normal_x - delta_pos_ned.y*normal_y > -1.2 && t1_-t0_ > 0.1) {
			
			// Use euclidean distance instead
			if ( delta_pos_ned.x*delta_pos_ned.x + delta_pos_ned.y*delta_pos_ned.y + delta_pos_ned.z*delta_pos_ned.z < switching_distance*switching_distance && t1_-t0_ > 0.1) {
				waypoint_copy(WP_GOAL, WP_WP6);
				waiting=false;
			}
		}
		else if (WP_EQUALS(WP_GOAL, WP_WP6)) {
			psi_ref = atan2(waypoints[WP_WP2].enu_f.x-waypoints[WP_WP1].enu_f.x, waypoints[WP_WP2].enu_f.y-waypoints[WP_WP1].enu_f.y);
			...
			// Use euclidean distance instead
			if ( delta_pos_ned.x*delta_pos_ned.x + delta_pos_ned.y*delta_pos_ned.y + delta_pos_ned.z*delta_pos_ned.z < switching_distance*switching_distance && t1_-t0_ > 0.1) {
				waypoint_copy(WP_GOAL, WP_WP7);
				waiting=false;
			}
		}
		else if (WP_EQUALS(WP_GOAL, WP_WP7)) {
			psi_ref = atan2(waypoints[WP_WP3].enu_f.x-waypoints[WP_WP2].enu_f.x, waypoints[WP_WP3].enu_f.y-waypoints[WP_WP2].enu_f.y);
			...
			// Use euclidean distance instead
			if ( delta_pos_ned.x*delta_pos_ned.x + delta_pos_ned.y*delta_pos_ned.y + delta_pos_ned.z*delta_pos_ned.z < switching_distance*switching_distance && t1_-t0_ > 0.1) {
				waypoint_copy(WP_GOAL, WP_WP8);
				waiting=false;
			}
		}
		else if (WP_EQUALS(WP_GOAL, WP_WP8)) {
			psi_ref = atan2(waypoints[WP_WP4].enu_f.x-waypoints[WP_WP3].enu_f.x, waypoints[WP_WP4].enu_f.y-waypoints[WP_WP3].enu_f.y);
		...
		///
		...
		else if (WP_EQUALS(WP_GOAL, WP_WP9)) {
			psi_ref = atan2(waypoints[WP_WP1].enu_f.x-waypoints[WP_WP4].enu_f.x, waypoints[WP_WP1].enu_f.y-waypoints[WP_WP4].enu_f.y);
			...
			// Use euclidean distance instead
			if ( delta_pos_ned.x*delta_pos_ned.x + delta_pos_ned.y*delta_pos_ned.y + delta_pos_ned.z*delta_pos_ned.z < switching_distance*switching_distance && t1_-t0_ > 0.1) {
				waypoint_copy(WP_GOAL, WP_WP10);
				waiting=false;
			}
		}
		else if (WP_EQUALS(WP_GOAL, WP_WP10)) {
			psi_ref = atan2(waypoints[WP_WP2].enu_f.x-waypoints[WP_WP1].enu_f.x, waypoints[WP_WP2].enu_f.y-waypoints[WP_WP1].enu_f.y);
			...
			// Use euclidean distance instead
			if ( delta_pos_ned.x*delta_pos_ned.x + delta_pos_ned.y*delta_pos_ned.y + delta_pos_ned.z*delta_pos_ned.z < switching_distance*switching_distance && t1_-t0_ > 0.1) {
				waypoint_copy(WP_GOAL, WP_WP11);
				waiting=false;
			}
		}
		else if (WP_EQUALS(WP_GOAL, WP_WP11)) {
			psi_ref = atan2(waypoints[WP_WP3].enu_f.x-waypoints[WP_WP2].enu_f.x, waypoints[WP_WP3].enu_f.y-waypoints[WP_WP2].enu_f.y);
			...
			// Use euclidean distance instead
			if ( delta_pos_ned.x*delta_pos_ned.x + delta_pos_ned.y*delta_pos_ned.y + delta_pos_ned.z*delta_pos_ned.z < switching_distance*switching_distance && t1_-t0_ > 0.1) {
				waypoint_copy(WP_GOAL, WP_WP12);
				waiting=false;
			}
		}
		else if (WP_EQUALS(WP_GOAL, WP_WP12)) {
			psi_ref = atan2(waypoints[WP_WP4].enu_f.x-waypoints[WP_WP3].enu_f.x, waypoints[WP_WP4].enu_f.y-waypoints[WP_WP3].enu_f.y);
	...
	///
		...
		else if (WP_EQUALS(WP_GOAL, WP_WP13)) {
			psi_ref = atan2(waypoints[WP_WP1].enu_f.x-waypoints[WP_WP4].enu_f.x, waypoints[WP_WP1].enu_f.y-waypoints[WP_WP4].enu_f.y);
			...
			// Use euclidean distance instead
			if ( delta_pos_ned.x*delta_pos_ned.x + delta_pos_ned.y*delta_pos_ned.y + delta_pos_ned.z*delta_pos_ned.z < switching_distance*switching_distance && t1_-t0_ > 0.1) {
				waypoint_copy(WP_GOAL, WP_WP14);
				waiting=false;
			}
		}
		else if (WP_EQUALS(WP_GOAL, WP_WP14)) {
			psi_ref = atan2(waypoints[WP_WP2].enu_f.x-waypoints[WP_WP1].enu_f.x, waypoints[WP_WP2].enu_f.y-waypoints[WP_WP1].enu_f.y);
			...
			// Use euclidean distance instead
			if ( delta_pos_ned.x*delta_pos_ned.x + delta_pos_ned.y*delta_pos_ned.y + delta_pos_ned.z*delta_pos_ned.z < switching_distance*switching_distance && t1_-t0_ > 0.1) {
				waypoint_copy(WP_GOAL, WP_WP15);
				waiting=false;
			}
		}
		else if (WP_EQUALS(WP_GOAL, WP_WP15)) {
			psi_ref = atan2(waypoints[WP_WP3].enu_f.x-waypoints[WP_WP2].enu_f.x, waypoints[WP_WP3].enu_f.y-waypoints[WP_WP2].enu_f.y);
			...
			// Use euclidean distance instead
			if ( delta_pos_ned.x*delta_pos_ned.x + delta_pos_ned.y*delta_pos_ned.y + delta_pos_ned.z*delta_pos_ned.z < switching_distance*switching_distance && t1_-t0_ > 0.1) {
				waypoint_copy(WP_GOAL, WP_WP16);
				waiting=false;
			}
		}
		else if (WP_EQUALS(WP_GOAL, WP_WP16)) {
			psi_ref = atan2(waypoints[WP_WP4].enu_f.x-waypoints[WP_WP3].enu_f.x, waypoints[WP_WP4].enu_f.y-waypoints[WP_WP3].enu_f.y);
		...
		///
		...
		else if (WP_EQUALS(WP_GOAL, WP_WP17)) {
			psi_ref = atan2(waypoints[WP_WP1].enu_f.x-waypoints[WP_WP4].enu_f.x, waypoints[WP_WP1].enu_f.y-waypoints[WP_WP4].enu_f.y);
			...
			// Use euclidean distance instead
			if ( delta_pos_ned.x*delta_pos_ned.x + delta_pos_ned.y*delta_pos_ned.y + delta_pos_ned.z*delta_pos_ned.z < switching_distance*switching_distance && t1_-t0_ > 0.1) {
				waypoint_copy(WP_GOAL, WP_WP18);
				waiting=false;
			}
		}
		else if (WP_EQUALS(WP_GOAL, WP_WP18)) {
			psi_ref = atan2(waypoints[WP_WP2].enu_f.x-waypoints[WP_WP1].enu_f.x, waypoints[WP_WP2].enu_f.y-waypoints[WP_WP1].enu_f.y);
			...
			// Use euclidean distance instead
			if ( delta_pos_ned.x*delta_pos_ned.x + delta_pos_ned.y*delta_pos_ned.y + delta_pos_ned.z*delta_pos_ned.z < switching_distance*switching_distance && t1_-t0_ > 0.1) {
				waypoint_copy(WP_GOAL, WP_WP19);
				waiting=false;
			}
		}
		else if (WP_EQUALS(WP_GOAL, WP_WP19)) {
			psi_ref = atan2(waypoints[WP_WP3].enu_f.x-waypoints[WP_WP2].enu_f.x, waypoints[WP_WP3].enu_f.y-waypoints[WP_WP2].enu_f.y);
			...
			// Use euclidean distance instead
			if ( delta_pos_ned.x*delta_pos_ned.x + delta_pos_ned.y*delta_pos_ned.y + delta_pos_ned.z*delta_pos_ned.z < switching_distance*switching_distance && t1_-t0_ > 0.1) {
				waypoint_copy(WP_GOAL, WP_WP20);
				waiting=false;
			}
		}
		else if (WP_EQUALS(WP_GOAL, WP_WP20)) {
			psi_ref = atan2(waypoints[WP_WP4].enu_f.x-waypoints[WP_WP3].enu_f.x, waypoints[WP_WP4].enu_f.y-waypoints[WP_WP3].enu_f.y);
	...
	// Single WP flight
	//state_nn[19] = 11200;//max_rpm_input_nn;
	// Consecutive WP flight	
	state_nn[20] = 12400;//max_rpm_input_nn;//
