 ...
 */
static inline void init_first_order_zoh_low_pass(struct FirstOrderZOHLowPass *filter, const float tau, const float sample_time,
    float value)
{
 ...
 * @param[in] sample_time sampling period of the signal [s]
 */
static inline void update_first_order_zoh_low_pass_tau(struct FirstOrderZOHLowPass *filter, const float tau, const float sample_time)
{
  filter->discrete_time_constant = exp( -sample_time / tau);
}
