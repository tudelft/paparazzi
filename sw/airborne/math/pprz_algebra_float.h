...
extern void float_rmat_of_axis_angle(struct FloatRMat *rm, const struct FloatVect3 *uv, float angle);
...
extern void float_rmat_integrate_fi(struct FloatRMat *rm, const struct FloatRates *omega, float dt);
 ...
 * a2c = a2b comp b2c , aka  a2c = a2b * b2c
 */
extern void float_quat_comp_norm_shortest(struct FloatQuat *a2c, const struct FloatQuat *a2b, const struct FloatQuat *b2c);

/** Composition (multiplication) of two quaternions with normalization.
 * a2b = a2c comp_inv b2c , aka  a2b = a2c * inv(b2c)
 */
extern void float_quat_comp_inv_norm_shortest(struct FloatQuat *a2b, const struct FloatQuat *a2c, const struct FloatQuat *b2c);

/** Composition (multiplication) of two quaternions with normalization.
 * b2c = a2b inv_comp a2c , aka  b2c = inv(_a2b) * a2c
 */
extern void float_quat_inv_comp_norm_shortest(struct FloatQuat *b2c, const struct FloatQuat *a2b, const struct FloatQuat *a2c);
 ...
 */
extern void float_quat_differential(struct FloatQuat *q_out, const struct FloatRates *w, float dt);

/** in place first order quaternion integration with constant rotational velocity */
extern void float_quat_integrate_fi(struct FloatQuat *q, const struct FloatRates *omega, float dt);

/** in place quaternion integration with constant rotational velocity */
extern void float_quat_integrate(struct FloatQuat *q, const struct FloatRates *omega, float dt);
...
/** a = 0 */
static inline void float_vect_zero(float *a, int n)
{
  int i;
  for (i = 0; i < n; i++) { a[i] = 0.; }
}
...
/** a = b */
static inline void float_vect_copy(float *a, const float *b, int n)
{
  int i;
...
/** o = a + b */
static inline void float_vect_sum(float *o, const float *a, const float *b, int n)
{
  int i;
...
/** o = a - b */
static inline void float_vect_diff(float *o, const float *a, const float *b, int n)
{
  int i;
  for (i = 0; i < n; i++) { o[i] = a[i] - b[i]; }
}
...
/** o = a * b (element wise) */
static inline void float_vect_mul(float *o, const float *a, const float *b, int n)
{
  int i;
  for (i = 0; i < n; i++) { o[i] = a[i] * b[i]; }
}
...
/** a += b */
static inline void float_vect_add(float *a, const float *b, int n)
{
  int i;
  for (i = 0; i < n; i++) { a[i] += b[i]; }
}
...
/** a -= b */
static inline void float_vect_sub(float *a, const float *b, int n)
{
  int i;
  for (i = 0; i < n; i++) { a[i] -= b[i]; }
}
...
/** o = a * s */
static inline void float_vect_smul(float *o, const float *a, float s, int n)
{
  int i;
...
/** o = a / s */
static inline void float_vect_sdiv(float *o, const float *a, float s, int n)
{
  int i;
  if (fabs(s) > 1e-5) {
    for (i = 0; i < n; i++) { o[i] = a[i] / s; }
...
/** ||a|| */
static inline float float_vect_norm(const float *a, int n)
{
  int i;
  float sum = 0;
...
/** a *= s */
static inline void float_vect_scale(float *a, float s, int n)
{
  int i;
  for (i = 0; i < n; i++) { a[i] *= s; }
}
...
/** a.b */
static inline float float_vect_dot_product(const float *a, const float *b, int n)
{
  int i;
  float dot = 0.f;
 ...
 * o: [m x 1]
 */
static inline void float_mat_vect_mul(float *o, float **a, const float *b, int m, int n)
{
  int i, j;
  for (i = 0; i < m; i++) {
    o[i] = 0;
...
/** o = I - v v^T */
static inline void float_mat_vmul(float **o, const float *v, int n)
{
  int i, j;
  for (i = 0; i < n; i++) {
    for (j = 0; j < n; j++) {
